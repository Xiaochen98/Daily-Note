当然可以。ROS2 的 launch（尤其是 Python launch）本质上就是：用代码把一堆节点怎么启动、参数怎么传、话题怎么 remap、条件怎么开关写清楚。
...
声明参数/开关（Launch Arguments）

创建节点（Node / ComposableNode）
...
写一个 launch：只启动 robot_state_publisher + lidar driver + imu + odom

在同一个 launch 里加：slam_toolbox / amcl（二选一条件启动）

最后 include Nav2 bringup：传入 map 和 params_file
